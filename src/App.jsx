...
function Button({ children, className = '', variant = 'primary', ...props }) {
  ...
  const variants = {
    ...
    subtle:
      'bg-white text-gray-700 ring-1 ring-gray-200 hover:bg-gray-50 focus-visible:ring-gray-400',
    success:
...
export default function App() {
  ...
  function pushLog(line) {
    setLog(l => [line, ...l].slice(0, 1000)) // keep most recent 1000 lines
  }
  ...
  async function handleBackup() {
    if (!hasPreview) {
      setToast({ kind: 'warn', msg: 'Run a preview first.' })
      return
    }
    setLoading(true)
    try {
  ...
  async function applyNow() {
    ...
    if (!backupId) {
      setToast({ kind: 'warn', msg: 'Create a backup first.' })
      return
    }
    setLoading(true)
    setSummary(null)
    ...
    try {
      // Optional: show a fake in-UI progress that increments while waiting
  ...
  async function handleRollback() {
    if (!backupId) {
      setToast({ kind: 'warn', msg: 'No backup to restore.' })
      return
    }
    setLoading(true)
    setToast({ kind: 'info', msg: 'Rolling back…' })
    try {
  ...
  // Clear preview when % changes to avoid stale UI
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-gray-200 bg-white p-3 shadow-sm sm:hidden">
        <div className="flex items-center gap-2">
          <Button variant="subtle" onClick={handlePreview} disabled={loading} className="flex-1">Preview</Button>
          <Button variant="warn" onClick={handleBackup} disabled={loading || !hasPreview} className="flex-1">Backup</Button>
          <Button variant="success" onClick={handleApply} disabled={loading || !backupId} className="flex-1">Apply</Button>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <Button variant="subtle" onClick={handlePreview} disabled={loading}>Preview</Button>
              <Button variant="warn" onClick={handleBackup} disabled={loading || !hasPreview}>Backup</Button>
              <Button variant="success" onClick={handleApply} disabled={loading || !backupId}>Apply</Button>
              <Button variant="danger" onClick={handleRollback} disabled={loading || !backupId}>Rollback</Button>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl px-4 pb-28 pt-6 sm:px-6 sm:pb-10">
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Stat label="New total" value={
              totals ? (
                <span className={totals.delta >= 0 ? 'text-emerald-600' : 'text-rose-600'}>
                  {totals.newSum.toLocaleString()}
                </span>
              ) : '—'
            }/>
          </div>
        ...
        <section className="mt-6 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            {!hasPreview && (
              <Button variant="subtle" onClick={handlePreview} disabled={loading}>Run Preview</Button>
            )}
          </div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
                    <div>
                      ...
                      {r.compare_at_price != null && (
                        <div className="text-xs text-gray-500">Compare: {r.compare_at_price}</div>
                      )}
                    </div>
                    <div className="text-right">
                      ...
                      {r.newCompare != null && (
                        <div className="text-xs text-gray-600">Compare: <b>{r.newCompare}</b></div>
                      )}
                    </div>
                  </div>
                </div>
